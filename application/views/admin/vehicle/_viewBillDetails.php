...
if (!empty($r)) {
    ...
    $balance         = $r['net_amount'] - $r['total_paid'];
}
    ...
    <div class="sh-info-grid">
        <div class="row g-0">
            <div class="col-6 col-md-3 sh-info-item">
                ...
                <div class="sh-info-value highlight"><?php echo $this->customlib->getSessionPrefixByType('ambulance_call_billing') . $r['id']; ?></div>
            </div>
            <div class="col-6 col-md-3 sh-info-item">
                <div class="sh-info-label"><?php echo $this->lang->line('patient_name'); ?></div>
                <div class="sh-info-value highlight"><?php echo html_escape(composePatientName($r['patientname'], $r['patient_id'])); ?></div>
            </div>
            <div class="col-6 col-md-3 sh-info-item">
        ...
        <div class="row g-0 sh-row-divider">
            ...
            <div class="col-6 col-md-3 sh-info-item">
                ...
                <div class="sh-info-value"><?php echo !empty($r['name']) ? composeStaffNameByString($r['name'], $r['surname'], $r['employee_id']) : '—'; ?></div>
            </div>
            <?php if (!empty($r['note'])) { ?>
        ...
        <div class="row g-0 sh-row-divider">
            <?php foreach ($fields as $fv) {
                $display = $r[$fv->name] ?? '';
                if ($fv->type == 'link') {
                    $display = ($display !== '') ? '<a href="' . html_escape($display) . '" target="_blank">' . html_escape($display) . '</a>' : '—';
                } else {
                    $display = html_escape($display ?: '—');
                }
            ?>
            <div class="col-6 col-md-3 sh-info-item">
                <div class="sh-info-label"><?php echo html_escape($fv->name); ?></div>
                <div class="sh-info-value"><?php echo $display; ?></div>
            </div>
            <?php } ?>
        </div>
        <?php } ?>
...
<div class="sh-vd-sum-wrap">
    <div class="sh-form-card h-100 overflow-hidden">
        ...
        <div class="sh-summary-row">
            ...
            <span><?php echo $currency_symbol . amountFormat($r['amount']); ?></span>
        </div>
        <div class="sh-summary-row">
        ...
        <div class="sh-summary-netamt">
            ...
            <span class="fw-bold fs-6"><?php echo $currency_symbol . amountFormat($r['net_amount']); ?></span>
        </div>
        <div class="sh-summary-row">
            <span class="text-secondary"><i class="fa fa-check-circle text-success me-1"></i><?php echo $this->lang->line('paid_amount'); ?></span>
            <span class="text-success fw-semibold"><?php echo $currency_symbol . amountFormat($r['total_paid']); ?></span>
        </div>
