            ...
            <div class="col-md-8">
                ...
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        <h3 class="card-title titlefix"><?php echo $this->lang->line('edit_menu_item'); ?></h3>
                    </div><!-- /.card-header -->
                    <!-- form start -->
...
?>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-check form-switch d-inline-flex m-0">
                                    <input id="ext_url" name="ext_url" type="checkbox" role="switch" class="form-check-input ext_url_chk" value="1" <?php echo set_checkbox('ext_url', '1', (set_value('ext_url', $result['ext_url'])) ? true : false); ?> />
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label d-block"><?php echo $this->lang->line('open_in_new_tab'); ?></label>
                                <div class="form-check form-switch d-inline-flex m-0">
                                    <input id="open_new_tab" name="open_new_tab" type="checkbox" role="switch" class="form-check-input chk" value="1" <?php echo set_checkbox('open_new_tab', '1', (set_value('open_new_tab', $result['open_new_tab'])) ? true : false); ?> />
                                </div>
                            </div>
                            <div class="mb-3">
                    ...
                    <div class="card-header ptbnull d-flex align-items-center">
                        ...
                        <div class="ms-auto d-flex gap-1">
                            ...
                            <a href="<?php echo site_url('admin/front/menus/additem/' . $m['slug']); ?>"
                               class="btn btn-sm <?php echo ($m['slug'] === $top_menu) ? 'btn-primary' : 'btn-outline-secondary'; ?>">
                                ...
                                <ol class="sortable">
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($listdropdown_Menus as $menu):
                                            $is_editing_parent = (isset($result['id']) && $menu['id'] == $result['id']);
                                            $has_editing_child = false;
                                            if (!empty($menu['submenus'])) {
                                                foreach ($menu['submenus'] as $sm) {
                                                    if (isset($result['id']) && $sm['id'] == $result['id']) { $has_editing_child = true; break; }
                                                }
                                            }
                                            $auto_open = $has_editing_child;
                                        ?>
                                            <li id="list_<?php echo $menu['id']; ?>">
                                                <div class="menu-item-row<?php if ($is_editing_parent) echo ' is-editing'; ?>">
                                                    <button type="button"
                                                ...
                                                <?php if (!empty($menu['submenus'])): ?>
                                                    <ol class="submenu-list" id="sub_<?php echo $menu['id']; ?>"<?php if ($auto_open) echo ' style="display:block;"'; ?>>
                                                        <?php foreach ($menu['submenus'] as $submenu_value):
                                                            $is_editing_sub = (isset($result['id']) && $submenu_value['id'] == $result['id']);
                                                        ?>
                                                            <li id="list_<?php echo $submenu_value['id']; ?>" class="no-nest">
                                                                <div class="sub-item-row<?php if ($is_editing_sub) echo ' is-editing'; ?>">
