...
/* ───────────────────────────────────────────────────────────────────────────
   Presentation values derived IN-VIEW from the existing $notifications array.
   The patient controller only passes $notifications, so — to keep the controller
   and all routes untouched (design-only) — we compute the day buckets, unread/read
   counts and the type→color map here, mirroring admin/systemnotification/index.php.
   ─────────────────────────────────────────────────────────────────────────── */
...
if (!empty($notifications)) {
    foreach ($notifications as $row) {
        ...
        /* read == 'no' means the row HAS been read (legacy convention); NULL/anything else = unread. */
        if (empty($row['read']) || $row['read'] != 'no') { $unread_count++; } else { $read_count++; }
    }
}
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="sh-notif-toolbar">
                ...
                <div class="sh-notif-chips" style="display:none;">
            ...
            <?php } else { ?>
                ...
                <?php foreach ($grouped as $bucket_key => $rows) {
                ...
                <div class="sh-notif-group" data-bucket="<?php echo $bucket_key; ?>">
                    ...
                    <div class="sh-notif-tl">
                        <?php foreach ($rows as $result) {
                            ...
                            $is_unread = empty($result['read']) || $result['read'] != 'no';
                            ...
                            <div class="sh-notif-card">
                                ...
                                <div class="sh-notif-expand">
                                    ...
                                    <div class="meta">
                                        ...
                                        <span class="sep">·</span>
                                        <span class="id">#N-<?php echo (int)$result['id']; ?></span>
                                        <span class="sep">·</span>
...
<script>
    ...
    $(function () {
        // ─── Auto-mark-read on item click ───
        // Removing .is-unread is what reveals .sh-notif-expand (CSS-driven expand animation).
        ...
        // ─── Mark all read — loops the single-item updateStatus endpoint (no bulk endpoint exists) ───
