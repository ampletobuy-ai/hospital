...
class Expense_model extends MY_Model
{
    ...
    public function search($text = null, $start_date = null, $end_date = null)
    {
        if (!empty($text)) {
            ...
            $this->db->join(
                "expense_head",
                "expenses.exp_head_id = expense_head.id",
            );
    ...
    public function add($data)
    {
        ...
        if (isset($data["id"]) && $data["id"] != "") {
            ...
            $message =
                UPDATE_RECORD_CONSTANT . " On Expenses id " . $data["id"];
            $action = "Update";
            $record_id = $data["id"];
            $this->log($message, $record_id, $action);
            //======================Code End==============================
    ...
    {
        $query =
            "SELECT sum(amount) as `amount` FROM `expenses` where date=" .
            $this->db->escape($date);
    ...
    public function getTotalExpenseBwdate($date_from, $date_to)
    {
        $object = new stdClass();
        $query =
            "SELECT sum(amount) as `amount` FROM `expenses` where date between " .
        ...
        $query2 =
            "SELECT sum(net_salary) as `amount` FROM `staff_payslip` where status = 'paid' and payment_date between " .
            $this->db->escape($date_from) .
            " and " .
            $this->db->escape($date_to);
    ...
    public function searchexpensegroup($search_type, $head_id = null)
    {
        ...
        $this->db
            ->select(
                'GROUP_CONCAT(expenses.id,"@",expenses.date,"@",expenses.name,"@",expenses.invoice_no,"@",expenses.amount) as expense, expense_head.exp_category,expenses.exp_head_id,sum(expenses.amount) as total_amount,' .
                    $field_variable,
            )
            ->from("expenses");
        $this->db->join(
    ...
    public function getAllRecord()
    {
        ...
        $this->datatables
            ...
            ->searchable(
                "expenses.name,expenses.invoice_no,expenses.date,expenses.note,expense_head.exp_category,generated_by_staff.name ,expenses.amount" .
                    $custom_field_column,
            )
            ->orderable(
                "expenses.name,expenses.invoice_no,expenses.date,expenses.note,expense_head.exp_category,generated_by_staff.name ,expenses.amount" .
            ...
            ->join(
                ...
                "generated_by_staff.id = expenses.generated_by",
                "left",
            )
    ...
    public function getexpensereport($start_date, $end_date, $head_id = null)
    {
        ...
        $this->datatables
            ...
            ->searchable(
                "expenses.name,expenses.invoice_no,expenses.date,expenses.note,expense_head.exp_category,expenses.amount" .
                    $custom_field_column,
            )
            ->orderable(
                "expenses.name,expenses.invoice_no,expenses.date,expenses.note,expense_head.exp_category,expenses.amount" .
