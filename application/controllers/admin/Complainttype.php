...
class Complainttype extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('complaint_type', $this->lang->line('complaint_type'), 'required|xss_clean');
    ...
    public function add_multiple_complaint_type() {
        ...
        if (isset($total_rows) && !empty($total_rows)) {
            foreach ($total_rows as $row_key => $row_value) {
                $this->form_validation->set_rules('complaint_type_' . $row_value, $this->lang->line('complaint_type'), 'trim|required|xss_clean');
            }
            if ($this->form_validation->run() == false) {
                $msg = array('complaint_type' => validation_errors());
            ...
            foreach ($total_rows as $row_key => $row_values) {
                $data = array(
                    'complaint_type' => $this->input->post('complaint_type_' . $row_values, TRUE),
            ...
            $json_array = array('status' => '2', 'error' => $this->lang->line('please_add_at_least_one_complaint_type'), 'message' => '');
            echo json_encode($json_array);
        }
    }
    ...
    public function edit()
    {
        ...
        $this->form_validation->set_rules('complaint_type', $this->lang->line('complain_type'), 'required|xss_clean');
        if ($this->form_validation->run() == false) {
            $msg = array(
